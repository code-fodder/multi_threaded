...
private:
    ...
    /// AMD.MONOTONIC NEW VARS ------------------------
    static std::mutex timers_mtx;
    static std::vector<timer *> timers;
    std::atomic<bool> time_updated {false};
    /// ---------------------------------

public:

    /// AMD.MONOTONIC NEW FUNCS -----------------------
    static void time_changed()
    {
        std::lock_guard<std::mutex> lk(timers_mtx);
        for (auto p_timer : timers)
        {
            p_timer->restart_for_time_update();
        }
    }

    void restart_for_time_update()
    {
        // Set the timer running to false to wake the timer and break out of the inner loop.
        // Set the time updated flag to true so that the timer restarts AND triggers the timer handler
        // instead of just stopping. Then wake the timer to process the flags
        timer_running = false;
        time_updated = true;
        cv.notify_all();
    }
    ...
    void add_timer()
    {
        ...
        //std::cout << this << " timer() - count: " << timers.size() << "\n";
    }
    ...
    timer()
    {
        add_timer(); // AMD.MONOTONIC - add to list
    }
    ...
    {
        stop();
        remove_timer(); // AMD.MONOTONIC - remove from list
    }
        ...
        bool oneshot = true)
    {
        ...
        /// Start the timer
        auto interval_dur = std::chrono::milliseconds(interval_ms);
        timer_running = true;
        timer_thread = std::thread([timeout_handler, interval_dur, oneshot, this]()
        {
            std::chrono::steady_clock::duration time_remaining {std::chrono::milliseconds(0)};
            // AMD.MONOTONIC - Outer loop for re-starting after a time change
            while (time_updated || timer_running)
            {
                // Setup the initial start time
                std::chrono::steady_clock::time_point next_time {std::chrono::steady_clock::now()};

                // Calculate the initial timeout timepoint, which is simply: now + interval or for a time change
                // its: now + time_remaining
                if (time_updated)
                {
                    next_time += time_remaining;
                    // Reset the flags
                    time_updated = false;
                    timer_running = true;
                }
                ...
                {
                    next_time += interval_dur;
                }

                // Keep running the timer until it is no longer running or is woken for time change
                while (timer_running)
                {
                    ...
                    {
                        // AMD.MONOTONIC - If the timer has stopped and this is not a time update, then stop the timer
                        // If the time is update, then carry on and run the timeout_handler and re-start the timer
                        if (!timer_running && !time_updated)
                        {
                            return;
                        }
                    }
                    ...
                    if (time_updated)
                    {
                        // Timer woken due to time change - calculate remaining time as
                        // interval - (now - start_time), where now - start_time is the amount of time already waited
                        time_remaining = interval_dur - (std::chrono::steady_clock::now() - start_wait_time);
                        // std::cout << "TIME LEFT: "
                        //     << std::chrono::duration_cast<std::chrono::milliseconds>(time_remaining).count()
                        //     << "ms\n";
                    }
                    else
                    {
                        ...
                        // if oneshot stop the timer - exit the thread
                        if (oneshot)
                        {
                            return;
                        }
